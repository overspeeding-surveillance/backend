...
class ManhattanDistanceTracker:
    """
    ...
    def update(self, results):
        ...
        for i in range(0, len(new_boxes)):
            ...
            for detection_id, info in self.detections.items():
                ...
                if abs(prev_center[0] - new_center[0]) + abs(prev_center[1] - new_center[1]) <= self.max_distance:
                    was_detected_in_previous_frame = True
                    final_response[detection_id] = {"box": new_box, "class": new_class_id}
                    break

            if was_detected_in_previous_frame is False:
                self.count = self.count + 1
                new_id = str(self.count)
                final_response[new_id] = {"box": new_box, "class": new_class_id}
